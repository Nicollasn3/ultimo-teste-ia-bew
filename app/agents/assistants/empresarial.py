...
class EmpresarialAgent(BaseAgent):
    """
    Agente especialista em Direito Empresarial.
    
    Base documental específica:
    - Direito Empresarial-Comercial (34 docs)
    - Arbitragem, Mediação e Conciliação (3 docs)
    ...
    def __init__(self, retriever: Optional[Any] = None):
        self.name = "Especialista_Empresarial"
        self.model = "gpt-4o-mini"
        self.instructions = SPECIALIST_PROMPTS_BY_AREA["empresarial"]
    ...
    def create_agent(self) -> Agent:
        return Agent(
            name=self.name,
            handoff_description="Especialista em Direito Empresarial (Brasil) - Sociedades, Contratos Empresariais, Recuperação, Arbitragem",
    ...
    async def run_with_trace(self, query, history=None):
        with trace("agente_especialista_empresarial"):
