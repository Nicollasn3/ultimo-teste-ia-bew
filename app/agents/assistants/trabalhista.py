...
class TrabalhistaAgent(BaseAgent):
    """
    Agente especialista em Direito do Trabalho.
    
    Base documental específica:
    - Direito do Trabalho (23 docs)
    - Processo do Trabalho (12 docs)
    + Banco Geral (Constitucional, Códigos, Prática Jurídica, etc.)
    ...
    def __init__(self, retriever: Optional[Any] = None):
        self.name = "Especialista_Trabalhista"
        self.model = "gpt-4o-mini"
        self.instructions = SPECIALIST_PROMPTS_BY_AREA["trabalhista"]
    ...
    def create_agent(self) -> Agent:
        return Agent(
            name=self.name,
            handoff_description="Especialista em Direito do Trabalho (Brasil) - CLT, Verbas, Processo Trabalhista, Rescisão",
    ...
    async def run_with_trace(self, query, history=None):
        with trace("agente_especialista_trabalhista"):
